...
const Points = () => {
    ...
    useEffect(() => {
      async function loadPosition(){
        //Pedimos a permissão do usuário para pegar a localização
        const { status } = await Location.requestPermissionsAsync();

        //Mandamos uma mensagem de erro (Alert importada do react native) caso não
        if(status !== 'granted'){
          Alert.alert('Oops', 'Precisamos de sua permissão para obter a localização!');
          return;
        }

        //Senão, capturamos a localizaçãoo atual
    ...
    //Use effect para pegar os pontos da API
    useEffect(() => {
      api.get('points', {
        params: {
